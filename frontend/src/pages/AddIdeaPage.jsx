...
const AddIdeaPage = () => {
  ...
  return (
    <Layout>
      <div className="max-w-2xl mx-auto bg-white rounded-xl p-6 shadow-sm border">
        <h1 className="text-xl font-semibold text-slate-800 mb-1">Share a new idea</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-1">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="relative">
                  ...
                  {showAIMenu && (
                    <div className="absolute right-0 mt-2 w-48 bg-white border rounded-xl shadow-xl z-20 py-1 overflow-hidden animate-fade-in-up" role="menu">
                      <button type="button" role="menuitem" onClick={() => handleEnhance("grammar")} className="w-full text-left px-4 py-2 text-xs hover:bg-indigo-50 transition focus:bg-indigo-50 focus:outline-none">Grammar Correction</button>
                      <button type="button" role="menuitem" onClick={() => handleEnhance("professional")} className="w-full text-left px-4 py-2 text-xs hover:bg-indigo-50 transition focus:bg-indigo-50 focus:outline-none">Make Professional</button>
                      <button type="button" role="menuitem" onClick={() => handleEnhance("creative")} className="w-full text-left px-4 py-2 text-xs hover:bg-indigo-50 transition focus:bg-indigo-50 focus:outline-none">Make Creative</button>
                      <button type="button" role="menuitem" onClick={() => handleEnhance("concise")} className="w-full text-left px-4 py-2 text-xs hover:bg-indigo-50 transition focus:bg-indigo-50 focus:outline-none">Make Concise</button>
                      <button type="button" role="menuitem" onClick={() => handleEnhance("expand")} className="w-full text-left px-4 py-2 text-xs hover:bg-indigo-50 transition focus:bg-indigo-50 focus:outline-none">Expand to Narrative</button>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="relative">
              <textarea
                ...
                onChange={handleChange}
                className={`w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 pr-10 transition-colors ${isAIProcessing ? "bg-indigo-50/30" : "bg-white"}`}
              ...
              {aiSuggestion && (
                <div className="absolute left-0 right-0 -bottom-10 bg-indigo-600 text-white text-[11px] p-2 rounded-lg flex items-center justify-between shadow-lg animate-fade-in-up z-10">
                  <div className="flex items-center gap-2">
            ...
            <div className="flex justify-between items-center mt-1">
              <span className={`text-[10px] ${form.description.length > 2000 ? "text-red-500 font-bold" : "text-slate-400"}`}>
            ...
            {showEmojiPicker && (
              <div className="absolute right-0 top-12 z-10 shadow-xl rounded-lg overflow-hidden border">
          ...
          {/* Attachments Section */}
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Attachments (Images, Docs)</label>
            <div className="border border-dashed border-slate-300 rounded-lg p-4 bg-slate-50 text-center">
              ...
              <span className="text-xs text-slate-400 block mt-1">Max 5 files (Images, PDF, Docs)</span>
            </div>
            
            {attachments.length > 0 && (
              <div className="mt-3 space-y-2">
                {attachments.map((file, index) => (
                  <div key={index} className="flex items-center justify-between bg-white border p-2 rounded-lg text-sm">
                    <span className="truncate text-slate-700 max-w-[80%]">{file.name}</span>
                    <button
                      type="button"
          ...
          <div className="pt-2 border-t border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <button
              ...
              className="flex items-center gap-2 text-sm text-slate-600 hover:text-indigo-600 transition min-h-[44px] px-2 rounded-lg hover:bg-slate-50"
            >
               <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-fade-in">
              ...
              <div className="p-4 space-y-6">
                {/* Visibility Section */}
                <div>
                   <h4 className="text-sm font-semibold text-slate-800 mb-2">Who can see your post?</h4>
                   <div className="space-y-2">
                     <label className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50 cursor-pointer border border-transparent hover:border-slate-200 transition">
                       <input 
                         type="radio" 
                         name="visibility" 
                         value="public"
                       ...
                       <div>
                         ...
                         <div className="text-xs text-slate-500">Anyone on or off IdeaCollab</div>
                       </div>
                     </label>
                     <label className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50 cursor-pointer border border-transparent hover:border-slate-200 transition">
                       <input 
                       ...
                       />
                       <div>
                         <div className="text-sm font-medium text-slate-700">Connections only</div>
                         <div className="text-xs text-slate-500">Connections on IdeaCollab</div>
                       </div>
                     </label>
                     <label className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50 cursor-pointer border border-transparent hover:border-slate-200 transition">
                       <input 
                       ...
                       />
                       <div>
                         <div className="text-sm font-medium text-slate-700">Private</div>
                         <div className="text-xs text-slate-500">Only you can see this post</div>
                       </div>
                     </label>
                   </div>
                </div>
                ...
                <div>
                   <h4 className="text-sm font-semibold text-slate-800 mb-2">Comment control</h4>
                   <div className="space-y-2">
                     <label className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50 cursor-pointer border border-transparent hover:border-slate-200 transition">
                       <input 
                         type="radio" 
                     ...
                     <label className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50 cursor-pointer border border-transparent hover:border-slate-200 transition">
                       <input 
                         ...
                         className="w-4 h-4 text-indigo-600 border-slate-300 focus:ring-indigo-500"
                       />
                       <div className="text-sm font-medium text-slate-700">No one</div>
                     </label>
                   </div>
                </div>
              </div>
