...
const AIChatBox = () => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      {/* Chat Window */}
      {isOpen && (
        <div className="bg-white w-80 sm:w-96 h-[500px] rounded-2xl shadow-2xl border border-slate-200 mb-4 flex flex-col overflow-hidden animate-fade-in-up">
          {/* Header */}
                ...
                <div
                  className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-sm shadow-sm ${
                    msg.sender === "user"
                      ...
                      : "bg-white text-slate-700 border border-slate-100 rounded-bl-none prose prose-slate prose-sm"
                  }`}
                >
                  {msg.sender === "ai" ? (
          ...
          <form onSubmit={handleSend} className="p-4 bg-white border-t border-slate-100">
            <div className="relative flex items-center">
              <input
                ...
                className="w-full bg-slate-100 border-0 rounded-xl pl-4 pr-12 py-3 text-sm focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all placeholder:text-slate-400"
              />
              <button
                type="submit"
