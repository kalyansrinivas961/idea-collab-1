...
const CommentSection = ({ idea, onUpdate }) => {
  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-sm border mt-4">
      <h2 className="font-semibold text-slate-800 mb-4">Comments ({idea.comments?.length || 0})</h2>
      ...
      {/* Add Comment Form */}
      {!user ? (
         <div className="text-center bg-slate-50 rounded-lg p-4 text-sm text-slate-500">
           Please <Link to="/login" className="text-indigo-600 font-medium">login</Link> to join the discussion.
         </div>
        ...
        <form onSubmit={handleSubmit} className="flex gap-3">
          ...
          <div className="flex-1 relative">
            <textarea
              ...
              className="w-full bg-slate-50 border-0 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500 focus:bg-white transition resize-none"
              rows="1"
              onKeyDown={(e) => {
            ...
            <button 
              ...
              className="absolute right-2 top-2 p-1 text-indigo-600 hover:bg-indigo-50 rounded-full disabled:opacity-50 transition"
            >
